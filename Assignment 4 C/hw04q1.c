/**
*
*Purpose:This program was ment to show how structs are used and how to read and write from a file
*
...
//sort function that will sort the array of structs well called
void sort() {
	struct studentRecord studentTemp = list[count - 1];
	int index = count - 1;
	//for loop to loop through the array
	for (int i = 0; i < count; i++) {
		// if last elements name goes before the current one alphabetically we want to put the last where the current on e is shift the other 
		if (stricmp(list[count - 1].studentName, list[i].studentName) <= 0) {
			if (i <= index) {
				index = i;
			}
		}
	...
	//put the last element in the right index
	list[index] = studentTemp;
...
int addSort(char* studentName_input, char* major_input, char* schoolYear_input, unsigned int studentID_input)
{
	...
	//if count is the same size as the max we know we cannot add anyone else into this array
	...
	{ 
		...
		for (int i = 0; i < count; i++) {
			if (stricmp(studentName_input, list[count].studentName ) == 0) {
				return 0;
			}
		}
		//else student isn't in the list so we add them 
		strcpy(list[count].studentName, studentName_input);
		strcpy(list[count].major, major_input);

		//since schoolyear is enum we have to pass the int associate with the enum this if block trys to math the enum with an int
		...
		//set the student id equal to the one provided
		list[count].studentID = studentID_input;

		//Increment count and call the sort  function
		count++;
		sort();
		
		// student added properly retrun 1
		return 1;
...
void display()
{
	// this for loop will loop through the array and print out the elements in this struct
	for (int i = 0; i < count; i++) {
		printf("Student Name: %s\n", list[i].studentName);
		printf("Major: %s\n", list[i].major);
		printf("School year: ");
		//if block determines which statement to print in correspsonce with what value is stored in list[i].schoolyear
		...
		if (list[i].schoolYear == 2) {
			printf("junior\n");
		}
		if (list[i].schoolYear == 3) {
			printf("senior\n");
		}
		printf("Student ID: %d\n", list[i].studentID);
		printf("\n");
...
void load(char* fileName)
{
	...
	int temp = -1; //creating a temp to hold the type integer

	fread(&count, sizeof(count), 1, file);		// First, store the number of students in the list

	// Parse the list and write student record to file
	// 
	for (int i = 0; i < count; i++)
	{
		//do freads to get in data to apprioriate place
		fread(list[i].studentName, sizeof(list[i].studentName), 1, file);
		fread(list[i].major, sizeof(list[i].major), 1, file);
		// convert int to a enum 
		fread( &temp, sizeof(temp), 1, file);
		if (temp == 0)
			list[i].schoolYear = freshman;		// 0 for freshman
		if (temp == 1)
